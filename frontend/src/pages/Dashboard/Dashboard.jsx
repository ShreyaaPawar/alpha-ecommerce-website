  ...
  useEffect(() => {
    const loadData = async() => {
      try{
        ...
        console.error(error);
        TransformStream.error('Unable to view the data');
      }finally{
        setLoading(false);
      }
    }
        ...
        <div className="stats-grid">
          <div className="stat-card">
            ...
            <div className="stat-content">
              <h3>Today's Sales</h3>
              <p>&#8377; {Number((data.todaySales.toFixed(2))).toLocaleString('en-IN')}</p>
            </div>
          </div>
              ...
              <tbody>
                {data.recentOrders.map((order) => (
                  <tr key={order.orderId}>
                    ...
                    <td>&#8377; {Number((order.grandTotal.toFixed(2))).toLocaleString('en-IN')}</td>       
                    <td>
